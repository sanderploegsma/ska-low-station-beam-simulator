...
separately. The Python simulator this ports from lives alongside this
module under `tango/` -- see its `CLAUDE.md` (repo root) for full
background.
...
root (see that workflow's `paths:` filter), then builds and publishes a
