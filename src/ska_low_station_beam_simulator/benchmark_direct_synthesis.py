...
candidate_counts = sorted(
    {1, 2, 4, 8, 16, 24, 32, max_numba_threads // 2, max_numba_threads}
)
candidate_counts = [n for n in candidate_counts if 1 <= n <= max_numba_threads]

all_results: dict[int, dict[int, dict]] = {}
for num_channels, label in [
    (96, "current (75 MHz)"),
    (MAX_NUM_CHANNELS, "full band (300 MHz)"),
]:
    print("=" * 70)
    print(
        f"{label}, NUM_CHANNELS={num_channels} -- tone + tiled noise + pulsar combined"
    )
    print("=" * 70)
...
print("=" * 70)
print(
    f"TARGET CHECK: ~8 CPU cores/pod, {MAX_NUM_CHANNELS} channels, tone+noise+pulsar combined"
)
print("=" * 70)
...
print(
    f"noise tile-bank memory (both pols): {streamer.bank_memory_bytes() / 1e9:.3f} GB"
)
...
print("=" * 70)
print(
    f"ONE-TIME CONSTRUCTION BUDGET (target 10s, hard limit 30s), {MAX_NUM_CHANNELS} channels"
)
print("=" * 70)
...
for n_tiles_check in (256, 512, 1024):
    ...
    flag = (
        "OK"
        if build_s <= 10.0
        else ("OVER 10s TARGET" if build_s <= 30.0 else "OVER 30s HARD LIMIT")
    )
    print(
        f"n_tiles={n_tiles_check:>4}  bank_mem={mem_gb:6.2f}GB  build={build_s:7.3f}s  [{flag}]"
    )
...
for period_s in (0.01, 0.05, 0.1, 0.2, 0.3):
    ...
    # now actually builds.
    n_period_samples_check = round(period_s * CHANNEL_OUTPUT_RATE_HZ)
...
for num_channels in (96, MAX_NUM_CHANNELS):
    best_threads = min(
        all_results[num_channels], key=lambda k: all_results[num_channels][k]["mean_ms"]
    )
